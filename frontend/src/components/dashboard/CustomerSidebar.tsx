          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors group ${isActive ? 'bg-indigo-600/20 text-white' : 'hover:bg-slate-900 hover:text-white'}`}
              >
                <item.icon className={`mr-3 h-5 w-5 ${isActive ? 'text-indigo-400' : 'text-slate-500 group-hover:text-indigo-400'}`} />
                {item.name}
              </Link>
            );
          })}
